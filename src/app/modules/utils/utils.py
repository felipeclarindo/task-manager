...
def lista_cronograma(lista_tarefas: list[dict]) -> list[dict]:
    ...
    for tarefa in lista_tarefas:
        ...
        if tarefa["PRIORIDADE"] == "em progresso":
            ...
            point_counter += 1
        if (
            datetime.strptime(tarefa["DATA_VENCIMENTO"], "%d/%m/%Y").date()
            - datetime.strptime(tarefa["DATA_CRIACAO"], "%d/%m/%Y").date()
        ).days < 4:
            point_counter += 2
        elif (
            datetime.strptime(tarefa["DATA_VENCIMENTO"], "%d/%m/%Y").date()
    ...
    lista_ordenada.sort(
        key=lambda x: (x["pontos"], -x["dias para entrega"]), reverse=True
    )
